...
# Define dimensions
base_side_length = 1.0  # Length of each side of the triangular base
height = 2.3           # Height of the pyramid
...
# Create solver - for FreeCAD 1.0.0 compatibility
solver = ObjectsFem.makeSolverCalculix(doc, "SolverCalculix")

# In FreeCAD 1.0, solver properties are handled differently
# We'll set them only if they exist
try:
    # Try to set solver properties if available
    ...
    # If properties can't be set, we'll just continue with default settings
...
# Assign material to the truss
try:
    # Older FreeCAD versions
    material.References = [(truss_obj, "Solid")]
except:
    try:
        # Newer FreeCAD versions
        matObj = analysis.addObject(ObjectsFem.makeMaterialMechanicalNonlinear(doc, material))[0]
        matObj.References = [(truss_obj, "Solid")]
    except:
        # Fallback - just add material reference directly from FemGui
...
print("To run the analysis in FreeCAD 1.0.0, you can use:")
print("1. Select the analysis object in the tree view")
print("2. Use the 'Solve' button in the task panel or menu")
print("3. After solving, right-click on the 'SolverCalculix' object and select 'Show result'")
